...
クロックを共有してプレゼンテーションタイムスタンプを受け取るのは、**OAAT** でグループ化された Tune エンドポイントだけです。DLNA、AirPlay、Chromecast など独立した出力のグループは一括操作には使えますが、Tune はそれらを同期済みマルチルームシステムとは表現しません。ゾーンごとの遅延は手動補正であり、音響レイテンシの自動測定ではありません。
