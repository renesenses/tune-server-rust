...
**OAAT**로 그룹화된 Tune 엔드포인트만 공통 시계를 사용하고 프레젠테이션 타임스탬프를 받습니다. DLNA, AirPlay, Chromecast 등 독립 출력 그룹은 함께 제어할 수 있지만 Tune은 이를 동기화된 멀티룸 시스템으로 표현하지 않습니다. 영역별 지연은 수동 보정이며 음향 지연을 자동으로 측정한 값이 아닙니다.
