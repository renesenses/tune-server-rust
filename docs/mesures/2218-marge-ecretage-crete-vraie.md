...
**Prouvé** (15 témoins verts, sur toute PR Rust) : les comportements du
tableau ci-dessus, tels qu'ils sont. Les 4 témoins `#[ignore]` restants sont
...
Ce que #4072 ne traite pas : B, C, D et E, dont les quatre témoins `#[ignore]`
restent rouges.
...
```sh
cargo test -p tune-core --test marge_et_crete_2218 -- --nocapture   # 15 verts, 4 ignorés
cargo test -p tune-core --test marge_et_crete_2218 -- --ignored      # 4 rouges : les défauts B–D
```
...
### Les cinq défauts — A corrigé, B à E toujours à trancher par Bertrand
