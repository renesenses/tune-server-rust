...
### 3.4.1 Garde-fous des capacités `playlists` / `streaming` / `kv` (#4716)
...
fusion de playlists : `tune_core::streaming::matching::apparier_chez_le_service`
(recherche + `best_stream_match_scored`), que la route
`POST /playlist-manager/transfer` appelle désormais elle aussi. Un seul verdict
pour l'écran et pour le greffon. La réponse porte `score` et `approximate`
(sous `MATCH_ACCEPT_SCORE`), à charge du greffon de ne pas écrire en silence un
appariement douteux chez un service.
...
- **P3.5** — #4716 : permissions `playlists`, `streaming`, `kv` (tranche 1 de
  l'épique #4715, greffon « Playlists converter »), avec les garde-fous du
  §3.4.1.
