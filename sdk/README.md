...
The idempotent migration preserves `zone_*_eq_profile`, crossfeed settings and presets. Since v0.9.156 the equalizer is an optional plugin: the migration no longer installs or enables it. It is installed from the plugin catalogue (`POST /api/v1/plugins/equalizer/install`, Free entitlement). When an existing configuration is found (`zone_*_eq_profile` or `eq_presets`) and the user has never installed or uninstalled the plugin, the migration only sets `plugin_equalizer_install_proposed=true`; no setting is changed or removed, and the saved profiles become active as-is once installed. `GET /api/v1/plugins/equalizer` exposes `installed`, `install_proposed` and `existing_configuration` at the root of the JSON. The source-composed provider stays compiled into the binary, so installing works offline and downloads nothing. The `dsp_eq` capability remains advertised as enabled for backward compatibility; crossfeed uses its own `crossfeed` entitlement. Existing Premium accounts retain all source-composed providers. The accepted policy is an immediate hard cut for crossfeed on Free accounts: playback and mutations are denied, while the exact saved settings remain in the database. The UI explains the change. After upgrading to Premium, reactivate crossfeed in Plugins to recover those settings; there is no grace period or Free editing mode. A migration marker prevents resurrection after explicit uninstall. This transition intentionally keeps reference implementations in the server build: removal of that compatibility build is a separate release policy, not a promised binary secrecy boundary.
...
`native` describes ABI-capable implementations, not a user installation requirement. `distribution: source` describes how their default provider is composed into the host. Each `bundled_in` field identifies an **unconditional Cargo dependency**: equalizer/crossfeed in `tune-core`, converter/declick in `tune-server`. Published binaries therefore already contain the EQ provider, including builds with `--no-default-features`; installing the equalizer from the catalogue needs no package, compiler, network download or restart of a download flow — it only sets the `plugin_equalizer_installed`/`_enabled` keys. Startup no longer enables it by itself. PURE and an explicit previous disable remain respected. Removing the provider from the binary is a separate, later release policy. `in_tree` is reserved here for feature-selected host plugins.

`--check` rejects missing inventory entries, stale expansions, unmanaged server build commands and missing/optional bundled dependencies. `python -m unittest discover -s scripts/tests -p test_plugin_catalog.py` adds a fixture plugin through the real Cargo declarations and proves propagation to all workflows; removing a feature from any generated list is rejected. Native workspace tests and package checks still validate actual code: registration alone does not implement a plugin or authorize a new production slot.
