...
--
-- Jumelle PostgreSQL de la migration SQLite 100. Les deux listes sont SEPAREES —
...
-- migration 057 — donc le demarrage — sur une base ou elle n'existe pas encore.
...
-- La migration la plus haute enregistre son numero. `run_pg_migrations`
-- n'ecrit rien dans `schema_version` : c'est le script qui pose sa ligne, et
-- seul le plus haut fait avancer `MAX(version)`. L'oublier laisserait la base
-- se croire en 56 a jamais et rejouerait ce script a chaque demarrage — le
-- defaut mesure sur la 052 (#3699).
INSERT INTO schema_version (version, name) VALUES (57, 'favoris_ordre_manuel')
