...
BEGIN
  ...
  LOCK TABLE public.streaming_favorites IN ACCESS EXCLUSIVE MODE;
  IF typ <> 'bigint' THEN
    ALTER TABLE public.streaming_favorites ALTER COLUMN id DROP DEFAULT;
    ALTER TABLE public.streaming_favorites ALTER COLUMN id TYPE BIGINT USING id::bigint;
  END IF;
  CREATE SEQUENCE IF NOT EXISTS public.streaming_favorites_id_seq;
  ALTER TABLE public.streaming_favorites ALTER COLUMN id
    SET DEFAULT nextval('public.streaming_favorites_id_seq');
  SELECT max(id) INTO max_id FROM public.streaming_favorites;
  SELECT last_value INTO sequence_value FROM public.streaming_favorites_id_seq;
  ...
  IF max_id >= sequence_value THEN
    PERFORM setval('public.streaming_favorites_id_seq', max_id, true);
  END IF;
END
$migration$;
