...
/// Le point d'accès en ligne, en production.
///
/// Passé en paramètre plutôt que lu ici : c'est la seule façon d'exercer
/// `reconcilier()` contre un cloud simulé. Même idiome que
/// `bio_sync::download_artist_bios`, qui reçoit aussi son URL.
...
/// Regarder, ou écrire.
///
/// Un second `bool` collé à `avec_pistes` s'échangerait sans un mot : à
/// l'appel les deux se ressemblent, et l'inversion transforme une simulation
/// en suppression de masse. Un type distinct rend l'échange impossible à
/// compiler — c'est la garde la plus forte, celle que le compilateur tient.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Mode {
    /// Calculer le plan et le journaliser, ne RIEN mettre en file. C'est le
    /// défaut de la route manuelle : on regarde avant de supprimer.
    ABlanc,
    /// Mettre les ordres de suppression en file. Demande explicite seulement.
    Appliquer,
}

impl Mode {
    pub fn est_a_blanc(self) -> bool {
        matches!(self, Mode::ABlanc)
    }
}
...
async fn ids_en_ligne(
    ...
    loop {
        let url = format!(
            "{base_api}/{server_id}/{}?per_page={PAR_PAGE}&page={page}",
            genre.chemin()
        );
...
async fn reconcilier_genre(
    ...
    let a_blanc = mode.est_a_blanc();
    let en_ligne = ids_en_ligne(http_client, base_api, server_id, access_token, genre).await?;
...
///
/// `mode` : [`Mode::ABlanc`] calcule et journalise le plan sans rien mettre en
/// file — le rapport dit alors ce qui SERAIT supprimé. C'est le mode par défaut
/// de la route manuelle : on regarde le plan avant de l'exécuter.
///
