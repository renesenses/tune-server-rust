...
/// Entrees encore en attente pour un type d'entite donne.
fn pending_count_of(backend: &Arc<dyn DbBackend>, entity_type: &str) -> i64 {
    backend
        .query_one(
            "SELECT COUNT(*) FROM sync_changelog WHERE synced = 0 AND entity_type = ?",
            &[&entity_type as &dyn ToSqlValue],
        )
        .ok()
        .flatten()
        .and_then(|row| row.first().and_then(|v| v.as_i64()))
...
pub async fn full_sync(
    ...
    let pending_albums = pending_count_of(backend, "album");
    ...
    if gap > 0 {
        warn!(
            ...
            tracks_pending = pending_count_of(backend, "track"),
