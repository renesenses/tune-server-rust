...
/// pré-gain automatique le dit : ici −8,59 dB pour un unique low-shelf à +8 dB
/// — la norme L1 du plateau (8,581 dB) plus la marge de troncature de #4594
/// (0,01 dB), réservée à la place de son seul gain crête (8,00 dB), parce
/// qu'un plateau dépasse son maximum fréquentiel sur un front.
...
async fn le_rapport_a_chaud_chiffre_le_format_et_le_pregain() {
    ...
    assert!((g + 8.591_233).abs() < 1e-5, "pré-gain gauche : {g}");
    assert!((d + 8.591_233).abs() < 1e-5, "pré-gain droit : {d}");
}
