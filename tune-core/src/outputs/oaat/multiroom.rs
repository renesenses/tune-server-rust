...
mod pacing_regression_tests {
    ...
    /// #2215 : seul OAAT partage l'horloge du contrôleur et reçoit des
    /// timestamps de présentation. Le contrat le dit, et il ne revendique
    /// AUCUNE précision chiffrée tant que la latence de rendu n'est pas
    /// mesurée sur du matériel.
    #[test]
    fn seul_oaat_annonce_le_contrat_d_horodatage_de_presentation() {
        ...
        assert_eq!(
            contract["render_latency_calibrated"], false,
            "la latence de restitution n'est pas mesurée : ne pas prétendre l'avoir calibrée"
        );
        assert!(
            contract["accuracy_claim_ms"].is_null(),
            "aucune précision chiffrée sans validation matérielle de bout en bout"
        );
    }
