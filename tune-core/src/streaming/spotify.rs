    ...
    fn map_album(item: &serde_json::Value) -> StreamAlbum {
        StreamAlbum {
            ...
            artist: item["artists"]
                ...
                .unwrap_or("")
                .into(),
            artist_id: item["artists"]
                .as_array()
                .and_then(|a| a.first())
                .and_then(|a| a["id"].as_str())
                .map(Into::into),
            cover_path: item["images"]
                .as_array()
                .and_then(|i| i.first())
                .and_then(|i| i["url"].as_str())
                .map(Into::into),
...
impl StreamingService for SpotifyService {
    ...
    fn enabled(&self) -> bool {
        self.enabled_override
            .unwrap_or(self.client_id != DEFAULT_CLIENT_ID)
    }
    ...
    async fn search(&self, query: &str, limit: usize) -> Result<SearchResults, String> {
        let data = self
            .api_get(&format!(
                "/search?q={}&type=track,album,artist&limit={limit}",
        ...
        Ok(SearchResults {
            ...
            albums: data["albums"]["items"]
                .as_array()
                .map(|i| i.iter().map(Self::map_album).collect())
                .unwrap_or_default(),
    ...
    async fn get_track(&self, id: &str) -> Result<StreamTrack, String> {
        self.api_get(&format!("/tracks/{id}"))
            .await
            .map(|d| Self::map_track(&d))
    }
    async fn get_track_url(&self, _id: &str, _q: Option<&str>) -> Result<StreamUrl, String> {
        Err("Spotify requires Connect/librespot for streaming".into())
    }
    async fn get_album(&self, id: &str) -> Result<StreamAlbum, String> {
    ...
    async fn get_artist(&self, id: &str) -> Result<StreamArtist, String> {
    ...
    async fn get_playlist(&self, id: &str) -> Result<StreamPlaylist, String> {
        ...
        Ok(StreamPlaylist {
            ...
            name: d["name"].as_str().unwrap_or("").into(),
            description: d["description"].as_str().map(Into::into),
            cover_path: d["images"]
                .as_array()
                .and_then(|i| i.first())
                .and_then(|i| i["url"].as_str())
    ...
    async fn get_artist_albums(&self, artist_id: &str) -> Result<Vec<StreamAlbum>, String> {
        let data = self
            .api_get(&format!(
                "/artists/{artist_id}/albums?include_groups=album,single&limit=50"
            ))
            .await?;
        Ok(data["items"]
            .as_array()
    ...
    async fn get_artist_top_tracks(&self, artist_id: &str) -> Result<Vec<StreamTrack>, String> {
        let data = self
            .api_get(&format!("/artists/{artist_id}/top-tracks"))
            .await?;
        Ok(data["tracks"]
            .as_array()
            .map(|i| i.iter().map(Self::map_track).collect())
    ...
    async fn get_genres(&self) -> Result<Vec<StreamGenre>, String> {
        ...
        Ok(data["categories"]["items"]
            .as_array()
            .map(|i| i.iter().map(Self::map_category_to_genre).collect())
            .unwrap_or_default())
    }
    ...
    async fn get_genre_albums(
        ...
        let data = self
            .api_get(&format!(
                "/browse/categories/{genre_id}/playlists?limit={limit}"
            ))
            .await?;
        let playlists: Vec<String> = data["playlists"]["items"]
            .as_array()
            .map(|i| {
                i.iter()
                    .filter_map(|p| p["id"].as_str().map(Into::into))
                    .collect()
            })
            .unwrap_or_default();
...
mod tests {
    ...
    fn map_track_basic() {
        ...
        assert_eq!(
            ...
            Some("http://img.spotify.com/cover.jpg")
        );
    ...
    fn map_album_basic() {
        ...
        assert_eq!(
            ...
            Some("http://img.spotify.com/album.jpg")
        );
    }
    ...
    fn map_artist_basic() {
        ...
        assert_eq!(
            ...
            Some("http://img.spotify.com/artist.jpg")
        );
    }
