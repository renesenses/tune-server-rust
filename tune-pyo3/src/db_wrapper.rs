...
fn artist_to_dict<'py>(py: Python<'py>, a: &Artist) -> PyResult<pyo3::Bound<'py, PyDict>> {
    ...
    if let Some(ref v) = a.sort_name {
        d.set_item("sort_name", v)?;
    }
    if let Some(ref v) = a.musicbrainz_id {
        d.set_item("musicbrainz_id", v)?;
    }
    if let Some(ref v) = a.bio {
        d.set_item("bio", v)?;
    }
    if let Some(ref v) = a.image_path {
        d.set_item("image_path", v)?;
    }
    Ok(d)
}
...
fn track_to_dict<'py>(py: Python<'py>, t: &Track) -> PyResult<pyo3::Bound<'py, PyDict>> {
    ...
    if let Some(v) = t.album_id {
        d.set_item("album_id", v)?;
    }
    if let Some(ref v) = t.album_title {
        d.set_item("album_title", v)?;
    }
    if let Some(v) = t.artist_id {
        d.set_item("artist_id", v)?;
    }
    if let Some(ref v) = t.artist_name {
        d.set_item("artist_name", v)?;
    }
    d.set_item("disc_number", t.disc_number)?;
    d.set_item("track_number", t.track_number)?;
    d.set_item("duration_ms", t.duration_ms)?;
    if let Some(ref v) = t.file_path {
        d.set_item("file_path", v)?;
    }
    if let Some(ref v) = t.format {
        d.set_item("format", v)?;
    }
    if let Some(v) = t.sample_rate {
        d.set_item("sample_rate", v)?;
    }
    if let Some(v) = t.bit_depth {
        d.set_item("bit_depth", v)?;
    }
    d.set_item("channels", t.channels)?;
    if let Some(ref v) = t.audio_hash {
        d.set_item("audio_hash", v)?;
    }
    d.set_item("source", &t.source)?;
    if let Some(ref v) = t.genre {
        d.set_item("genre", v)?;
    }
    if let Some(ref v) = t.composer {
        d.set_item("composer", v)?;
    }
    if let Some(ref v) = t.musicbrainz_recording_id {
        d.set_item("musicbrainz_recording_id", v)?;
    }
    Ok(d)
}
...
impl RustDatabase {
    ...
    fn artist_get_or_create<'py>(
        ...
        let repo = ArtistRepo::new(self.db.clone());
        let a = repo
            .get_or_create(name, musicbrainz_id, sort_name)
            .map_err(pyo3::exceptions::PyRuntimeError::new_err)?;
        artist_to_dict(py, &a)
    }
    ...
    fn artist_search<'py>(
        ...
        artists
            .iter()
            .map(|a| Ok(artist_to_dict(py, a)?.into()))
            .collect()
    }
    ...
    fn track_search<'py>(
        ...
        tracks
            .iter()
            .map(|t| Ok(track_to_dict(py, t)?.into()))
            .collect()
    }
