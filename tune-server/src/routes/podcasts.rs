...
async fn rf_shows(
    ...
    let api_key = get_rf_api_key(&state)
        .ok_or_else(|| AppError::bad_request("radiofrance_api_key not configured in settings"))?;
    let api = RadioFranceApi::with_client(state.http_client.clone(), api_key);

    let station_code = q.station.as_deref().unwrap_or("FRANCEINTER");
    let station = RfStation::from_code(station_code)
        .ok_or_else(|| AppError::bad_request(&format!("unknown station: {station_code}")))?;

    match api.list_shows(station).await {
        Ok(shows) => Ok(Json(json!({
            "station": station.label(),
            "count": shows.len(),
            "shows": shows,
        }))),
        Err(e) => {
            warn!(station = station_code, error = %e, "rf_shows_failed");
            Err(AppError::internal(e))
        }
    }
}
...
async fn rf_search_shows(
    ...
    match api.search_shows(&q.q).await {
        ...
        Err(e) => {
            warn!(query = %q.q, error = %e, "rf_search_shows_failed");
            Err(AppError::internal(e))
        }
    }
}
