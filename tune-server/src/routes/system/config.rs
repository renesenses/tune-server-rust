...
pub(super) async fn update_config(
    ...
    // Corps commun aux trois sorties. #1627 y pose l'écho du mode ReplayGain
    // réellement appliqué ; #2149 l'ENRICHIT ensuite de ce que le patch a fait
    // aux racines. Un même `PATCH` peut porter les deux champs : il doit
    // répondre sur les deux, sinon le correctif #1627 redevient muet dès qu'un
    // `music_dirs` accompagne la demande.
    ...
    let Some(racines_apres) = patch_music_dirs else {
        return Ok(Json(reponse).into_response());
    };
    ...
    if retirees.is_empty() {
        reponse["music_dirs_removed"] = json!([]);
        return Ok(Json(reponse).into_response());
    }
    ...
    reponse["music_dirs_removed"] = json!(retirees);
    reponse["orphan_tracks"] = json!(plan.tracks);
...
mod purge_hors_perimetre_tests {
    ...
    /// Le témoin de la FUSION avec #1627 : `PATCH /system/config` porte
    /// maintenant DEUX correctifs qui écrivent chacun dans la réponse.
    ///
    /// Avant la résolution, chaque branche construisait son propre
    /// `json!({"ok": true})` et le rendait aussitôt. Garder l'une ou l'autre
    /// telle quelle rendait l'autre muette sans casser une seule compilation :
    /// un corps portant les deux champs perdait, selon la branche retenue,
    /// soit le compte d'orphelines de #2149, soit l'écho du mode de #1627.
    /// Ce test est ce qui empêche cette perte de repasser inaperçue.
    #[tokio::test]
    async fn un_seul_patch_repond_sur_music_dirs_et_sur_replaygain() {
        let state = etat();
        racines(&state, &["/musique/rock", "/musique/rock2"]);
        piste(&state, "/musique/rock/album/01.flac");
        // Le témoin anti-régression du préfixe reste en place : `rock2` est
        // conservé, ses pistes ne doivent jamais être comptées.
        piste(&state, "/musique/rock2/album/01.flac");

        let rep = patcher(
            &state,
            serde_json::json!({
                "music_dirs": [n("/musique/rock2")],
                "replaygain_source": "tags_then_analysis",
            }),
        )
        .await;
        ...
        assert_eq!(
            rep["music_dirs_removed"].as_array().map(|v| v.len()),
            Some(1),
            "#2149 doit encore NOMMER la racine retirée quand #1627 partage le corps : {rep}"
        );
        assert_eq!(
            rep["orphan_tracks"].as_i64(),
            Some(1),
            "une seule piste orpheline — celle de rock2 reste sous une racine configurée : {rep}"
        );
        assert_eq!(
            rep["replaygain_source"].as_str(),
            Some("tags_then_analysis"),
            "#1627 doit encore rendre l'écho du mode posé quand #2149 partage le corps : {rep}"
        );

        // Et l'écho ne ment pas : les deux axes existants ont bien été écrits,
        // sans qu'aucune ligne `replaygain_source` n'apparaisse en base.
        let settings = SettingsRepo::with_backend(state.backend.clone());
        assert_eq!(
            settings
                .get("replaygain_analysis_enabled")
                .unwrap()
                .as_deref(),
            Some("true"),
            "le mode « calcul » doit poser l'analyse"
        );
        assert_eq!(
            settings.get("replaygain_source").unwrap(),
            None,
            "`replaygain_source` reste une vue, jamais une ligne persistée"
        );
        // Rien n'a été supprimé : cette route ne supprime pas, #1627 non plus.
        assert_eq!(compte(&state), 2);
    }
