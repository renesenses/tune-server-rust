...
///   "volume": 0.708, "local_output_only": false,
...
/// `compensation_db` : ce que la compensation DEMANDE quand l'interrupteur
/// est ouvert, 0 sinon. `local_output_only` : faux depuis #5071 — une zone
/// réseau la reçoit aussi. `applied_by` dit PAR OÙ : `output_volume` (sortie
/// locale, par le volume raboté à l'unité) ou `stream_gain` (zone réseau, gain
/// cuit dans le flux après l'égaliseur, borné à la crête : il peut rendre
/// MOINS que `compensation_db` sur une piste dont les crêtes n'ont pas la
/// place).
///
