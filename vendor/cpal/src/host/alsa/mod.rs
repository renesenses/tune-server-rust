...
pub struct Stream {
    ...
    // Retain the read end through Drop even when a disconnected worker exits.
    // This is the ownership prerequisite used by CPAL 0.18's early exit.
...
fn input_stream_worker(
    ...
    loop {
        let flow = poll_descriptors_and_prepare_buffer(&rx, stream, &mut ctxt, false)
            .unwrap_or_else(|err| {
        ...
        match flow {
            ...
            PollDescriptorsFlow::Disconnected => {
...
fn output_stream_worker(
    ...
    loop {
        let flow = poll_descriptors_and_prepare_buffer(&rx, stream, &mut ctxt, true)
            .unwrap_or_else(|err| {
                error_callback(err.into());
                PollDescriptorsFlow::Continue
            });
...
fn poll_descriptors_and_prepare_buffer(
    ...
    if res == 0 {
        if recover_output_errors {
            return Ok(recover_output_pcm_state(stream)?.unwrap_or(PollDescriptorsFlow::Continue));
        }
        return Err(BackendSpecificError {
            description: String::from("`alsa::poll()` spuriously returned"),
        });
    }
    ...
    // POLLERR can be the first indication of xrun/suspend. Do not return
    // before querying ALSA's state: that used to bypass recovery indefinitely.
    if recover_output_errors && revents.intersects(alsa::poll::Flags::HUP | alsa::poll::Flags::NVAL)
    {
        return Ok(PollDescriptorsFlow::Disconnected);
    }
    if revents.contains(alsa::poll::Flags::ERR) {
        if !recover_output_errors {
            return Err(BackendSpecificError {
                description: String::from("`alsa::poll()` returned POLLERR"),
            });
        }
        if let Some(flow) = recover_output_pcm_state(stream)? {
            return Ok(flow);
        }
        // State may still be Running: avail() below can classify EPIPE.
    }

    // Check if data is ready for processing (either input or output)
    if !revents.intersects(alsa::poll::Flags::IN | alsa::poll::Flags::OUT)
        && !(recover_output_errors && revents.contains(alsa::poll::Flags::ERR))
    {
...
// Output-only adaptation of CPAL 0.18 ALSA recovery to the 0.17 error API.
// A pending resume is retried by the worker, keeping its drop pipe effective.
fn recover_output_pcm_state(
    stream: &StreamInner,
) -> Result<Option<PollDescriptorsFlow>, BackendSpecificError> {
    match stream.channel.state() {
        alsa::pcm::State::Disconnected => Ok(Some(PollDescriptorsFlow::Disconnected)),
        alsa::pcm::State::XRun => Ok(Some(PollDescriptorsFlow::XRun)),
        alsa::pcm::State::Suspended => match stream.channel.resume() {
            Ok(()) => Ok(Some(PollDescriptorsFlow::Continue)),
            Err(err) if err.errno() == libc::EAGAIN => Ok(Some(PollDescriptorsFlow::Continue)),
            Err(err) if err.errno() == libc::ENOSYS => Ok(Some(PollDescriptorsFlow::XRun)),
            Err(err) => Err(err.into()),
        },
        // In particular, do not turn an unexplained POLLERR into blind prepare().
        _ => Ok(None),
    }
}
