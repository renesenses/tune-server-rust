    ...
    T: Sample,
{
    ...
    let mut sum = T::zero();
    // Compensated summation preserves the small sinc tails when generating
    // large f32 tables. A plain sum biases every coefficient by up to 5e-5
    // for Tune's downsampling kernels (#4079). This runs only at construction;
    // the table size and per-sample interpolation remain unchanged.
    ...
    for (x, w) in window.iter().enumerate().take(totpoints) {
        ...
        let corrected = val - compensation;
