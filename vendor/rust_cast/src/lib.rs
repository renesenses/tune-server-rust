...
/// [TUNE PATCH] Chromecast devices (notably 1st/2nd gen and Chromecast Audio)
/// present a self-signed X.509 **v1** certificate. `rustls-webpki` refuses to
/// even parse v1 certificates (`UnsupportedCertVersion`), so the stock
/// `verify_tls12_signature`/`verify_tls13_signature` helpers — which parse the
/// end-entity certificate to extract its public key — fail with
/// `invalid peer certificate: Other(OtherError(UnsupportedCertVersion))`
/// before this verifier's `verify_server_cert` (which accepts everything)
/// even matters.
///
/// Since this verifier is only used by `connect_without_host_verification`
/// (certificate identity is already NOT verified, LAN-only usage), verifying
/// the handshake signature against an unauthenticated certificate adds no
/// real security. We therefore fall back to accepting the signature when the
/// certificate cannot be parsed by webpki; certificates that DO parse keep
/// the standard signature check.
fn accept_unparseable_cert(
    result: Result<HandshakeSignatureValid, rustls::Error>,
) -> Result<HandshakeSignatureValid, rustls::Error> {
    match result {
        Err(rustls::Error::InvalidCertificate(
            CertificateError::BadEncoding | CertificateError::Other(_),
...
impl ServerCertVerifier for NoCertificateVerification {
    ...
    fn verify_tls12_signature(
        ...
        accept_unparseable_cert(verify_tls12_signature(
            message,
            cert,
            dss,
    ...
    fn verify_tls13_signature(
        ...
        accept_unparseable_cert(verify_tls13_signature(
            message,
            cert,
            dss,
            &default_provider().signature_verification_algorithms,
